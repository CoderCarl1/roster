import { Address, Appointment, Customer, Prisma } from '@prisma/client';
import { address_create } from '~/models/address.server';
import { appointment_create, appointment_create_many, appointment_delete, appointment_find_many, appointment_findbyAddress, appointment_findbyCustomer, appointment_update } from '~/models/appointment.server';
import { customer_create, customer_delete_many, customer_find } from '~/models/customer.server';
...
describe('APPOINTMENT FUNCTIONS', () => {
    ...
    let mockAppointmentData = appointments[ 0 ];
    let validAppointmentData = {} as Appointment;

    let multipleApppointmentArray = [ appointments[ appointmentRef++ ], appointments[ appointmentRef++ ] ];

    beforeEach(async () => {
        customerRef = 0;
        addressRef = 0;
        appointmentRef = 1;
        multipleApppointmentArray = [ appointments[ appointmentRef++ ], appointments[ appointmentRef++ ] ];

        await customer_delete_many('test.com');
        await customer_delete_many('example.com');
        const createdCustomer = await customer_create(customers[ customerRef++ ]) as Customer;
        validCustomerId = createdCustomer.id;
        const createdAddress = await address_create({ ...addresses[ addressRef++ ], customerId: validCustomerId }) as Address;
        validAddressId = createdAddress.id;
        const createdAppointments = await appointment_create_many(validCustomerId, validAddressId, multipleApppointmentArray) as Appointment[];
    ...
    describe('SINGLE - ', () => {
        ...
        it('appointment_create returns an error when customerId is missing', async () => {
            ...
            const result = await appointment_create(invalidCustomerId, validAddressId, mockAppointmentData) as AppointmentOperationError;
        ...
        it('appointment_create returns an error when addressId is missing', async () => {
            ...
            const result = await appointment_create(validCustomerId, invalidAddressId, mockAppointmentData) as AppointmentOperationError;
            expect(result).toBeInstanceOf(AppointmentOperationError);
            expect(result?.message).toBe('Failed creating appointment');
        });
        it('appointment_create returns an error when prisma create throws or fails to create', async () => {
            jest.spyOn(prisma.appointment, 'create').mockRejectedValueOnce(new Error());
            const errorResult = await appointment_create(validCustomerId, validAddressId, mockAppointmentData);
            expect(errorResult).toBeInstanceOf(AppointmentOperationError);

            // @ts-ignore
            jest.spyOn(prisma.appointment, 'create').mockResolvedValueOnce(null!);
            const nullResult = await appointment_create(validCustomerId, validAddressId, mockAppointmentData);
        ...
        it('appointment_findbyCustomer returns appointments for a customer', async () => {
            ...
            expect((result as Appointment[]).length).toBe(multipleApppointmentArray.length);
        });
        it('appointment_findbyCustomer returns AppointmentOperationError if throws', async () => {
            jest.spyOn(prisma.appointment, 'findMany').mockRejectedValueOnce(new Error());
            const errorResult = await appointment_findbyCustomer(validCustomerId);
            expect(errorResult).toBeInstanceOf(AppointmentOperationError);
        });
        ...
        it('appointment_update returns the updated appointment', async () => {
            const appointment = await prisma.appointment.findFirst({ where: { id: validAppointmentId } });
            ...
            const result = await appointment_update(validAppointmentId, { ...mockAppointmentData, completed: true }) as Appointment;
        ...
        it('appointment_update returns AppointmentOperationError if throws', async () => {
            jest.spyOn(prisma.appointment, 'update').mockRejectedValueOnce(new Error());
            const errorResult = await appointment_update(validAddressId, validAppointmentData);
            expect(errorResult).toBeInstanceOf(AppointmentOperationError);
        });
        it('appointment_update returns the updated appointment', async () => {
            const result = await appointment_delete(validAppointmentId) as Appointment;
            expect(result).toMatchObject({
                id: validAppointmentId,
                customerId: validCustomerId,
                recurring: validAppointmentData.recurring,
    ...
    describe('MANY - ', () => {
        ...
        it('appointment_create_many returns an array of appointments', async () => {
            const appointmentDataArray = [appointments[appointmentRef++], appointments[appointmentRef++], appointments[appointmentRef++]]
            const result = await appointment_create_many(validCustomerId, validAddressId, appointmentDataArray) as Appointment[];
        ...
        it('appointment_create_many returns an AppointmentOperationError if throws', async () => {
            ...
            const errorResult = await appointment_create_many(validCustomerId, validAddressId, multipleApppointmentArray);
            expect(errorResult).toBeInstanceOf(AppointmentOperationError);
