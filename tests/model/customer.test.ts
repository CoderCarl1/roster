...
describe('CUSTOMER FUNCTIONS', () => {
    ...
    describe('SINGLE -', () => {
        ...
        it('customer_create accepts address to associate with customer', async () => {
            ...
            const createdRecord = (await customer_create(
                mockCustomerData,
                mockAddressData
            )) as Customer;
            expect(createdRecord).toMatchObject({
            ...
            if ('addresses' in createdRecord) {
                const addresses = createdRecord.addresses as Address[];

                expect(addresses).toHaveLength(mockAddressData.length);
        ...
        it('customer_find accepts 2nd parameter to list included association models', async () => {
            const { id } = (await customer_create(customers[customerRef], [
                addresses[addressRef++],
                addresses[addressRef++],
            ])) as Customer;
            const mockData = customers[customerRef];
        ...
        it('customer_update returns an updated customer', async () => {
            ...
            const customer = await customer_update(
                id,
        ...
        it('customer_update accepts an address and returns updated customer with associated addresses', async () => {
            ...
            const updatedAddressData = [
                { ...addresses[addressRef++], id: createdAddress.id },
            ];

            const updatedCustomer = (await customer_update(
                id,
                updatedCustomerdata,
                updatedAddressData
            )) as Customer;
        ...
        it('customer_update returns an error if no customerId received', async () => {
            ...
            const result = (await customer_update(
                '',
                updatedCustomerdata,
                updatedAddressData
            )) as CustomerOperationError;
        ...
        it('customer_delete handles invalid customerIds', async () => {
            ...
            expect(result?.message).toBe(
                'unable to delete invalid-customer-id'
            );
        });
    });
    ...
    describe('MANY -', () => {
        ...
        it('customer_delete_many deletes customers based on contact search string', async () => {
            ...
            const customerCount = await prisma.customer.count({
                where: { contact: { endsWith: 'test.com' } },
            });
        ...
        it('customer_delete_many handles errors during deletion', async () => {
            const mockError = new CustomerOperationError(
                'unable to delete customers'
            );
