...
describe('DATE FUNCTIONS', () => {
  ...
  describe('parseDate', () => {
    ...
    test('should throw an error for an invalid date string', () => {
      ...
      expect(() => parseDate(invalidDateString)).toThrow('Invalid date string provided.');
    });

    test('should throw an error for an invalid date object', () => {
      const invalidDateObject = new Date('invalid-date-string');

      expect(() => parseDate(invalidDateObject)).toThrow('Invalid date provided.');
    });

    test('should throw an error for an invalid input type', () => {
      const invalidInput = 42; // Not a string or Date

      expect(() => parseDate(invalidInput)).toThrow('Invalid date type provided.');
    });
  });
  describe('formatDate', () => {
  ...
  describe('getDaysInMonth', () => {
    ...
    test('should throw an error for an invalid month (less than 0)', () => {
      expect(() => getDaysInMonth(2021, -1)).toThrow('Invalid month provided.');
    });

    test('should throw an error for an invalid month (greater than 11)', () => {
      expect(() => getDaysInMonth(2021, 12)).toThrow('Invalid month provided.');
    });
  });
  ...
  describe('getDateRange', () => {
    ...
    test('should handle invalid dates gracefully', () => {
      ...
      expect(result).toEqual({ from: 'Invalid Date', to: 'Invalid Date' });
    });
