...
type DateInputProps = { children?: React.ReactNode, className?: string } & React.HTMLProps<HTMLDivElement>;
export default function DateInputs({ children, className = "", ...props }: DateInputProps) {
...
function Inputs() {
  ...
  function handleValueChange(calendarType: string, element?: HTMLInputElement | null, value?: number) {
    ...
    if (element) {
      ...
      setCalendarDate(new Date(DATE_VALUES.year, DATE_VALUES.month, DATE_VALUES.day));
      return;
    }

    if (!element && value) {
      if (!ref.current) return;
      const inputElement = ref.current!.querySelector(`input[name="${calendarType}"]`) as HTMLInputElement;
    ...
    function _handleMin(calendarType: string) {
      ...
      const { daysInMonth } = dates.dateParts(new Date(DATE_VALUES.year, DATE_VALUES.month, 1));
  ...
  function handleFocus(event: React.KeyboardEvent<HTMLInputElement>) {
    ...
    switch (event.key) {
      case 'Tab':
        event.shiftKey ? _focusPrevElement(inputName) : _focusNextElement(inputName);
        break;
      ...
      case 'Backspace':
        ...
        break;
      case 'ArrowUp':
        handleValueChange(currentEl.name, null, parseInt(currentEl.value) + 1)
        break;
      case 'ArrowDown':
        handleValueChange(currentEl.name, null, parseInt(currentEl.value) - 1)
        break;
