...
export default function WeekView() {
    ...
    function todayIsInWeek(week: visibleDayType[]) {
        if (
            dates.startOfWeek(new Date()).toDateString() ===
            dates.startOfWeek(week[0].date).toDateString()
        ) {
            return true;
        }
        return undefined;
            ...
            <tbody>
                {visibleDates?.length
                    ? visibleDates.map((week, weekIndex) => {
                          const weekClasses = isWithinWeek(week[0].date)
                              ? 'selected'
                              : '';
                                  ...
                                  <td>
                                      <Button
                                          className={joinClasses(
                                              'calendar__selection--weeks week',
                                              weekClasses
                                          )}
                                          onClick={() =>
                                              handleCalendarWeekSelect(week)
                                          }
                                          aria-label={`${dates.humanReadable(
                                              week[0].date
                                          )} to ${dates.humanReadable(
                                              week[6].date
                                          )}`}
                                          aria-selected={isWithinWeek(
                                              week[0].date
                                          )}
