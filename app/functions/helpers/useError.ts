 ...
 */
const useError = (initialState: Record<string, boolean> = {}, autoClearTimeouts?: true): UseErrorResult => {
  const errorTimeouts = useRef<Record<string, null | NodeJS.Timeout>>({});
  const [ showError, setShowError ] = useState<Record<string, boolean>>(initialState);
  ...
  */
  const handleError = (inputName: string): void => {
    if (autoClearTimeouts && errorTimeouts.current && errorTimeouts.current[ inputName ]) {
