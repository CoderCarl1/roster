...
export function useCaptureFocus<T extends HTMLElement>({
    cb,
    initialRef,
}: useCaptureFocusProps<T>): React.MutableRefObject<T | null> {
    ...
    useEffect(() => {
        function handleKeyPress(event: KeyboardEvent) {
            ...
            if (event.key === 'Escape') {
                ...
                const elementsArray = [
                    ...(domRef.current?.querySelectorAll(
                        'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'
                    ) as NodeListOf<HTMLElement>),
                ];

                if (elementsArray.length) {
                    const currentIndex = elementsArray.indexOf(
