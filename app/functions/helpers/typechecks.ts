...
export function isAddress(value: ObjTypes): value is TAddressWithCustomerNameAndFullAddress {
  ...
  if (value?.appointments){
    ...
    const appointmentsOnlyContainsAppointment = value.appointments.every(isAppointment)
    if (!isArray || !appointmentsOnlyContainsAppointment) {
      log({color: "blue"}, "isAddress appointments is", {data: {
        isArray, appointmentsOnlyContainsAppointment
      }})
      return false;
    }
  }
  ...
  return Boolean(
    exists('id', value) 
    && (value?.number && exists('number', value) || isNull(value?.number, `number is not null, ${stringObject(value)}`)) 
    && exists('line1', value) 
    && (value?.line2 && exists('line2', value) || isNull(value?.line2, `line2 is not null, ${stringObject(value)}`)) 
    && exists('suburb', value) 
    && exists('createdAt', value) 
    && (value?.updatedAt && exists('updatedAt', value) || isNull(value?.updatedAt, `updatedAt is not null, ${stringObject(value)}`)) 
    && isBool(value?.archived, 'archived is not a bool') 
     // customer
    && (value?.customerId && exists('customerId', value) || isNull(value?.customerId, `customerId is not null, ${stringObject(value)}`)) 
...
export function isAppointment(value: ObjTypes): value is TAppointmentWithCustomerNameAndFullAddress {
  ...
  return Boolean(exists('id', value)
    && isBool(value?.recurring, `recurring is not a bool`)
    && value?.frequency && exists('frequency', value) || isNull(value?.frequency, `frequency is not null`)
    && exists('customerId', value)
    && exists('addressId', value)
    && exists('start', value)
    && exists('end', value)
    && isBool(value?.completed, `completed is not a bool`)
    && (value?.completedAt && exists('completedAt', value) || isNull(value?.completedAt, `completedAt is not null`))
    && exists('createdAt', value)
    && (value?.updatedAt && exists('updatedAt', value) || isNull(value?.updatedAt, `updatedAt is not null`))
...
export function isCustomer(value: ObjTypes): value is TCustomer {
  ...
  if (value?.addresses){
    ...
    if (!isArray || !addressesOnlyContainsAddress) {
      log({color: "blue"}, "isCustomer addresses is", {data: {
        isArray, addressesOnlyContainsAddress
      }})
      return false;
    }
  }
  ...
  return Boolean(
    ...
    && (value?.suspendedAt && exists('suspendedAt', value) || isNull(value?.suspendedAt, `suspendedAt is not null, ${stringObject(value)}`)) 
    && exists('createdAt', value) 
    && (value?.updatedAt && exists('updatedAt', value) || isNull(value?.updatedAt, `updatedAt is not null, ${stringObject(value)}`)) 
...
function exists(key: string, object: unknown, message: string = `missing ${key} from object`) {
  if (!isObject(object)) {
    return false;
  }
  if (object && key in (object as Record<string, unknown>) && (object as Record<string, unknown>)[ key ]) {
    return true;
  }
...
function isNull(value: unknown, message: string = "value is not null") {
  if (value === null) {
    return true;
  }
  log(message)
...
function isObject(possibleObject: unknown) {
  if (typeof possibleObject !== 'object' || possibleObject === null || !possibleObject || Array.isArray(possibleObject)) {
    return false;
  }
